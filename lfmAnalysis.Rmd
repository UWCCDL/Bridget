...
Then, we are going tu run another mixed level model, but this time we are going to include a participant-level random _slope_, which accounts for the fact that different participants might benefit to different extends from the mean effect of the condition. Response times have a complicated relationship to accuracy, since they are the sum of retrieval times and perceptual-motor non-retrieval times.

```{r}
rt_model <- lmer(log(rt) ~ condition # Fixed effect of condition 
...
ggplot(cleandata_rt_agg, aes(x=condition, y=RT, color=condition)) +
  ...
  geom_text(data=cleandata_rt_summary, 
            aes(x=condition, y=RT + 300, label=paste(round(RT), "ms")), hjust= -0.1) +
  theme_minimal()
```